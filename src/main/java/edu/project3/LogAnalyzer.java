...
public class LogAnalyzer {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < parsedArds.size(); i++) {
            switch (parsedArds.get(i)) {
                case "--path":
                    path = parsedArds.get(i + 1);
                    break;
                case "--from":
                    from = convert(parsedArds.get(i + 1));
                    fromStr = parsedArds.get(i + 1);
                    break;
                case "--to":
                    to = convert(parsedArds.get(i + 1));
                    toStr = parsedArds.get(i + 1);
                    break;
                case "--format":
                    fileFormat = parsedArds.get(i + 1);
                    break;
                default:
            }
        }
